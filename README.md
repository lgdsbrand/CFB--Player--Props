...
| | |
|---|---|
| [docs/runbook.md](docs/runbook.md) | Every job: what it writes, when it runs, and what to do when it does not |
| [docs/configuration.md](docs/configuration.md) | The 15 `app_config` keys and every environment variable |
| [docs/schema.md](docs/schema.md) | Schema reference and the two rules it exists to enforce |
| [docs/odds.md](docs/odds.md) | What The Odds API actually serves, measured — CLAUDE.md §9.1 |
| [docs/calibration-report.html](docs/calibration-report.html) | The Phase 3 deliverable (generated by `run_backtest`) |
| [docs/odds-coverage-probe.md](docs/odds-coverage-probe.md) | Raw probe output (generated by `probe_odds` — overwritten each run) |

## Status

**Phases 1–4 complete; Phase 5 in progress.** Odds ingestion, the weekly AI-read
seam, the client reskin and pipeline monitoring have all shipped.
...
`ai_adapter` was switched to `"gemini"` on 2026-08-03 and reverted the same day:
the key resolves to Gemini's free tier (20 requests/day), which trains on
submitted content. It needs a key issued from the project that has billing.
...
The board is filled by `run_projections`, which writes one distribution per
player-market and a pick wherever there is a line to call against:

```bash
python -m worker.jobs.run_projections --season 2025 --weeks 10
```

Yardage markets show the model's lean until a book posts; anytime TD is callable
immediately because its line is structural (over 0.5). That split is the
late-line behaviour CLAUDE.md §7 requires. Full details, and every other job, in
...
worker/               Python 3.11 → Render cron
  ...
  worker/adapters/    CFBD adapter; the layer the NFL build replaces.
                      odds/, ai/ and alerts/ are pluggable seams, each chosen
                      by an `app_config` row rather than an import
  worker/jobs/        entrypoints, `python -m worker.jobs.<name>`
  tests/              pytest

supabase/migrations/  numbered SQL, the source of truth for the schema
docs/                 schema, runbook, configuration, odds findings (index above)
...
Six layers, in increasing strength. The last two are the standing rules: run
`audit_data` at the end of every phase, and let the docs tests fail rather than
letting a runbook go quietly out of date.
...
**Standing rule: run this at the end of every phase, and after any ingest.** The
defects it exists for are all silent ones — an endpoint truncating at 2,000 rows,
ids arriving as strings so a join matched nothing, bowl games stored as week 1
and leaking into every earlier week. None of them raised an error.

### 6. Are the docs still true?

```bash
cd worker && pytest tests/test_docs.py
```

Documentation is the one artefact with no natural failure mode: code that names
a renamed job crashes, and a runbook that names a renamed job reads perfectly.
These tests pin [docs/runbook.md](docs/runbook.md) to the jobs that exist and to
`render.yaml`, and [docs/configuration.md](docs/configuration.md) to the
`app_config` keys the migrations actually insert. Offline.
...
- **Worker → Render.** Uses [render.yaml](render.yaml): nine crons and one
  ...
  exits non-zero on anything critical so Render's own cron-failure notification
  is the backstop.
